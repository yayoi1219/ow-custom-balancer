...
| 対応言語       | 日本語 / English / 한국어 / 简体中文（画面右上で切替） |
...
- **API・権限・有効期限**（`test/api.test.ts`）
  ...
  - 指定した言語でエラー文面・バリデーション詳細・チーム分けの失敗理由を返すこと
- **多言語対応**（`test/i18n.test.ts`）
  - 全言語の辞書が正本（日本語）と同じ構造であること、空文字の翻訳が残っていないこと
  - `ja-JP` / `zh-Hans-CN` のような地域つきタグの解決、`Accept-Language` の品質値の扱い
  - Zod が訳文ではなくキーを返し、未知のキーが内部名を漏らさないこと
  - チーム分けの失敗理由・ランク表記・Discord 用テキストが言語に追従すること
...
日本語・英語・韓国語・簡体中文に対応しています。ヘッダー右上のセレクトボックスで切り替えられ、
選択内容は `localStorage` に保存されます。初回は `navigator.languages` から自動判定し、
対応していない言語の場合は日本語になります。

### 辞書の構成

| ファイル                   | 役割                                             |
| -------------------------- | ------------------------------------------------ |
| `src/shared/i18n/ja.ts`    | **正本**。この構造がそのまま `Messages` 型になる |
| `src/shared/i18n/en.ts`    | 英語（`Messages` 型に適合させる）                |
| `src/shared/i18n/ko.ts`    | 韓国語                                           |
| `src/shared/i18n/zh.ts`    | 簡体中文                                         |
| `src/shared/i18n/types.ts` | 対応言語の一覧・表示名・言語判定                 |
| `src/shared/i18n/index.ts` | 辞書の取得、ランク表記やエラーコードの変換       |

`ja.ts` を正本とし、他の言語は `Messages` 型に適合させています。そのためキーの追加漏れ・
削除漏れは `npm run typecheck` で失敗します。`test/i18n.test.ts` では、
関数と文字列の取り違えや空文字の翻訳が残っていないかも実行時に検証しています。

### 言語を1つ増やすには

1. `src/shared/i18n/types.ts` の `LOCALES` / `LOCALE_LABELS` / `LOCALE_HTML_LANG` / `resolveLocale()` に追加
2. `ja.ts` と同じ形の辞書ファイルを作り、`const xx: Messages = { ... }` として書く
3. `src/shared/i18n/index.ts` の `BUNDLES` に登録

型チェックが通れば、翻訳漏れはありません。

### サーバーが返す文面

チーム分けやドラフトの失敗理由には参加者名やロール名が入るため、サーバー側で文章を組み立てています。
クライアントは全リクエストに `X-OWCB-Locale` ヘッダーを付け、Worker はその言語（無ければ
`Accept-Language`、それも無ければ日本語）で応答します。Zod のバリデーションメッセージは
訳文ではなく `displayName.tooLong` のような安定したキーを返し、応答を組み立てる直前に訳文へ変換します。

### 法的文面の正本

プライバシーポリシーと利用規約は**日本語版を正本**とします。非日本語版にはその旨の注記を表示します
（翻訳と原文で解釈が分かれた場合に、どちらが優先されるかを明示するためです）。
...
│   │   ├── i18n/                  多言語辞書（ja が正本 / en・ko・zh）
│   │   ├── constants.ts           サービス名・重み・制限値などの定数
│   │   ├── ranks.ts               ランク ↔ 連続整数の変換
│   │   ├── types.ts               ドメイン型・API型
│   │   ├── errors.ts              エラーコード（文面は i18n 側）
...
│       ├── hooks/                 WebSocket 同期・公開設定取得・表示言語
│       └── lib/                   API クライアント・localStorage・整形
└── test/                          Vitest（balancer / validation / api / i18n ほか）
```
